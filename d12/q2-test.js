...
function reduceKnownDamage (spring, setup, level) {
  ...
  if (verbose) console.log({ index, length })
  ...
  for (let i = 0; i < setup.length; ++i) {
    ...
    for (let offset = length - targetCount; offset <= 0; ++offset) {
      ...
      if (verbose) console.log({ i, offset, newIndex })
      if (
        [...spring.slice(newIndex, newIndex + length)].every(canBeDamage) &&
        // . before - must be true
        // . after
        (newIndex + targetCount == spring.length || canBeOp(spring[newIndex + targetCount]))
      ) {
        // split spring / setup into 2 parts
        let newSpring = spring.slice(0, newIndex)
        ...
        if (verbose && level === 0) console.log({ add: possibilities0 * possibilities1 })
...
async function main () {
  ...
  for await (const line of rl) {
    ...
    let spring2 = spring.replace(/\.+/g, '.').replace(/^\.|\.$/g, '')
