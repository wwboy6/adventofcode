...
function reduceKnownDamage(spring, setup, level) {
  ...
  for (var i = 0; i < setup.length + 1; ++i) {
    ...
    for (var offset = length - targetCount; offset <= 0; ++offset) {
      ...
      if (_toConsumableArray(spring.slice(newIndex, newIndex + targetCount)).every(canBeDamage) && ( // . before - must be true
      // . after
      newIndex + targetCount == spring.length || canBeOp(spring[newIndex + targetCount]))) {
        ...
        var newSpring = spring.slice(0, Math.max(0, newIndex - 1));
        ...
        var total = possibilities0 * possibilities1;
        if (verbose && level === 0) console.log('==========', {
          total: total
        });
        sum += total;
    ...
    while (1) {
      switch (_context.prev = _context.next) {
        ...
        case 13:
          ...
          spring2 = "".concat(spring2, "?").concat(spring2, "?").concat(spring2, "?").concat(spring2, "?").concat(spring2);
          setupArr = [].concat(_toConsumableArray(setupArr), _toConsumableArray(setupArr), _toConsumableArray(setupArr), _toConsumableArray(setupArr), _toConsumableArray(setupArr)); // TODO: double
          ...
          console.log("".concat(data.spring, " ").concat(data.setup, " ").concat(count));
          sum += count;
