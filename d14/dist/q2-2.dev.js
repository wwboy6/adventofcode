...
var _require2 = require('lodash'),
    memoize = _require2.memoize; // const inputFilePath = 'input-test.txt'
...
var weight = function weight(hash) {
...
var slide = memoize(function (hash) {
  var data = hashToData(hash);
  ...
  return dataToHash(data);
});
var rotate = memoize(function (hash) {
  var data = hash.split('\n');
  var newData = [];
  var lineCount = data.length;
  var width = data[0].length;
  ...
  return regeneratorRuntime.async(function coreFunction$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          result = slide(data); // await recordOutput([data], result)
...
function main() {
  var file, rl, data, _iteratorNormalCompletion, _didIteratorError, _iteratorError, _iterator, _step, _value, line, hash, cycleCount, result, cycleCache, cycle, targetHash, lastCycleCount, c, newHash, dir;
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 45:
          ...
          targetHash = cycleCache[targetHash];
        ...
        case 52:
          ...
          for (c = 0; c < lastCycleCount; ++c) {
            hash = cycleCache[hash];
          }
        ...
        case 59:
          ...
          _context2.next = 62;
          return regeneratorRuntime.awrap(coreFunction(hash));

        case 62:
          newHash = _context2.sent;
          // slide
          newHash = rotate(newHash); // debugLog(data.map(line => line.join('')).join('\n'));
          // debugLog("===")
        ...
        case 67:
          ...
          // debugLog({result})
          // debugLog("===")
          cycleCache[hash] = newHash;
          hash = newHash; // debugLog({cycle})
